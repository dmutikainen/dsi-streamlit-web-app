...
age = st.number_input(
    ...
    min_value = 18,
    max_value = 120,
    value = 35) # value is prepopulated value
...
if st.button("Submit For Prediction"):
    ...
    pred_proba = model.predict_proba(new_data)[0][1] # the raw output from the precit_method is a bit nested, to extract just the probability we need to index
    
    # output prediction
    
    st.subheader(f"Bsed on these customer attributes, our model predicts a purchase probability of {pred_proba:.0%}")
